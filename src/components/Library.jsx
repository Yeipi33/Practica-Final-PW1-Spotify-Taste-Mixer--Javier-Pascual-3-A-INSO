"use client";
import { TbPlaylist } from "react-icons/tb";
import { AiOutlinePlus } from "react-icons/ai";
import ArtistWidget from "./widgets/ArtistWidget"; 
import DecadeWidget from "./widgets/DecadeWidget";
import GenreWidget from "./widgets/GenreWidget";
import MoodWidget from "./widgets/MoodWidget";
import PopularityWidget from "./widgets/PopularityWidget";
import TrackWidget from "./widgets/TrackWidget";

export default function Library() {
  const onClick = () => {
  };

  return (
    <div className="flex flex-col">
      <div className="flex items-center justify-between px-5 pt-4">
        <div className="inline-flex items-center gap-x-2">
          <TbPlaylist className="text-neutral-400" size={26} />
          <p className="text-neutral-400 font-medium text-md">
            Tu biblioteca
          </p>
        </div>
        <AiOutlinePlus 
          onClick={onClick}
          size={20}
          className="text-neutral-400 cursor-pointer hover:text-white transition"
        />
      </div>
      
      <div className="flex flex-col gap-y-2 mt-4 px-3">
        <p className="text-xs text-neutral-500 px-2 italic">Tus Artistas</p>
        <ArtistWidget name="Bad Bunny" image="/images/artist1.jpg" />
        <ArtistWidget name="Rosalía" image="/images/artist2.jpg" />
      </div>
    </div>
  );
}